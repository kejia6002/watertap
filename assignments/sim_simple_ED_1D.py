# this is a file for simulate a ED with a feed,
# separetor, edstack, disposal, and product
...
"""
This flowsheet implements a single-stack electrodialysis desaliantion system.
I have a solution , containing 5g/L NaCl.  I have a total flow rate of 5.2e-4 m3/s
"""
__author__ = "Kejia Hu"
...
def main():
    ...
    m.fs.feed.flow_mol_phase_comp[0, "Liq", "H2O"].fix(28.74)
    m.fs.feed.flow_mol_phase_comp[0, "Liq", "Na_+"].fix(4.449e-2)
    m.fs.feed.flow_mol_phase_comp[0, "Liq", "Cl_-"].fix(4.449e-2)
    ...
    m.fs.EDstack.cell_pair_num.setub(100000)
...
def build():
    ...
    m.fs.arc1 = Arc(source=m.fs.sepa0.to_dil_in, destination=m.fs.EDstack.inlet_diluate)
    m.fs.arc2 = Arc(
        source=m.fs.sepa0.to_conc_in, destination=m.fs.EDstack.inlet_concentrate
    )
    m.fs.arc3 = Arc(source=m.fs.EDstack.outlet_diluate, destination=m.fs.prod.inlet)
    m.fs.arc4 = Arc(source=m.fs.EDstack.outlet_concentrate, destination=m.fs.disp.inlet)
...
def initialize_system(m, solver=None):
    ...
    m.fs.sepa0.split_fraction[0, "to_conc_in1"].fix(
        max(2 - value(m.fs.recovery_vol_H2O) ** -1, 1e-8)
    )
    m.fs.sepa0.split_fraction[0, "to_conc_in1"].unfix()
    iscale.calculate_scaling_factors(m)
